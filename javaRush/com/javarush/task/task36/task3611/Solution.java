...
public class Solution {
    ...
    public Set<Integer> getAllFriendsAndPotentialFriends(int index, int deep) {
        ...
        //выбираем интексы людей, с кем дружит человек под индексом = index
        for (int i = 0; i < humansRelationships.length; i++) {
            ...
            else if (humansRelationships[i][index]){
                set.add(i);
                set.addAll(getAllFriendsAndPotentialFriends(i, deep - 1));
            }
        }
        ...
        //убираем из множества объект = index. т.к. он показыает на нашего человека под интексом = index
        set.remove(index);
